<template>
  <pa-row>
    ...
    <pa-col><pa-button @click="clickCustomNotification" type="primary"> Custom Notification</pa-button></pa-col>
  </pa-row>
</template>
...
function clickDanger() {
  M_Notification.danger("操作失败");
}
function clickInfo() {
  M_Notification.info("操作提示");
}
function clickWarning() {
  M_Notification.warning("操作警告");
}

function clickCustomNotification() {
  M_Notification({
    title: "提示",
    message: "这是一条带有标题的通知",
    duration: 5000,
    onClick: () => {
      M_Message.success("通知点击");
    },
    onClose: () => {
      M_Message.danger("通知关闭");
    }
  });
}
</script>
