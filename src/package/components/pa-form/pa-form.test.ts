...
const PaSelectMock = defineComponent({
  ...
  emits: ["update:modelValue", "change"],
  template:
    '<select class="pa-select" :disabled="disabled" :value="modelValue" @change="$emit(\'change\', $event.target.value)"><option v-for="opt in exOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option></select>'
});
