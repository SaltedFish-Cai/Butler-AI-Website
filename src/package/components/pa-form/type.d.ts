...
export type PaFormProps = {
   ...
   * @example
   * ```ts
   * <pa-form v-model:value={{ name: "张三", age: 18 }} />
   * ```
   * */
  data?: FormDataType;
   ...
   * @example
   * ```ts
   * <pa-form contrastData={{ name: "张三", age: 18 }} />
   * ```
   * */
  contrastData?: {};
   ...
   * @description 当设置该值为 `Array<PaFormItemType>` 时，会使用该值作为表单结构
   * @example
   * ```ts
   * <pa-form structure={[{ label: "姓名", prop: "name" }]} />
   * ```
   * */
   ...
   * @example
   * ```ts
   * <pa-form exDependent={{ disabledRule: { name: (value) => value === "张三" } }} />
   * ```
   * */
   ...
   * @type `PaFormCellExDependentType.select_RequestApi` 为 `(params: { query: string }) => Promise<Array<PaOptionType.Select>>`
   * @type `PaFormCellExDependentType.file_attachedData` 为 `{ [x: string]: Record<string, string> }`
   * @type `PaFormCellExDependentType.time_disabledDateFn` 为 `(date: any) => boolean`
   * @type `PaFormCellExDependentType.time_shortcuts` 为 `DatePickerShortcut[]`
   ...
   * @type `option` `PaOptionType.Select` 表单结构项
   * @description 当设置该值为 `(prop, value, oldValue) => void` 时，会使用该值作为回调函数
   * @example
   * ```ts
   * <pa-form @formCellChange={({prop, value, oldValue, option}) => { console.log(prop, value, oldValue, option) }} />
   * ```
   * */
   ...
   * @example
   * ```ts
   * <pa-form @formDataChange={(data) => { console.log("formDataChange", data) }} />
   * ```
   * */
...
//   tabsFormConfig: (PaFormChildType & { spanStyle?: spanStyleType })[];
// };
...
export type PaFormCellExDependentType = {
  /**
   * **选择器请求Api**
   * @type `(params: { query: string }) => Promise<PaOptionType.SelectList>`
   * @description 当设 cellConfig.type=='select' 时，会使用该值作为选择器请求Api
   * */
  select_RequestApi?: { [x: string]: ({ query }: { query: string }) => Promise<PaOptionType.SelectList> };
  /**
   * **文件上传请求Api**
