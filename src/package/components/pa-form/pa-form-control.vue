...
/**
 * **组件属性**
 * @description PaFormControl 组件的属性对象
 */
const props = withDefaults(
  defineProps<{
    ...
    /**
     * **表单规则**
     * @type `Record<string, `[`FormItemRule`](#formitemrule)` | `[`FormItemRule`](#formitemrule)`[]>` | `undefined`
     * @default `undefined`
     * @description 表单的验证规则
     */
     ...
     * @type `boolean`
     * @default `false`
     * @description 当设置该值为 `true` 时，会在 Tabs 表单中使用该组件
     */
    inTabsForm?: boolean;
  }>(),
  {
 ...
 * @description 存储各字段的验证错误信息
 */
const errorsMessage = ref<Record<string, string>>({});
/**
 * **验证规则存储**
 * @type `Ref<Record<string, `[`FormItemRule`](#formitemrule)` | `[`FormItemRule`](#formitemrule)`[]>>`
 * @description 存储表单各字段的验证规则
 */
const formRules = ref<Record<string, FormItemRule | FormItemRule[]>>(props.rules || {});
/**
 * **验证状态存储**
 * @type `Record<string, { state: "" | "error" | "success" | "validating"; message: string }>`
 * @description 存储各字段的验证状态信息
 */
const validateStates = reactive<Record<string, { state: "" | "error" | "success" | "validating"; message: string }>>({});

/**
 * **表单上下文**
 * @description 表单上下文对象，提供给子组件使用
 */
