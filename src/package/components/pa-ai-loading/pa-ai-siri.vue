<template>
  <div :id="renderId" ref="containerRef" class="ai-siri" :class="{ 'ai-siri--done': !loading }">
    ...
    <div class="ai-siri__overlay">
      <div v-if="loading" class="ai-siri__content">
        <p v-if="text" class="ai-siri__text">{{ text }}</p>
        <p v-if="subtext" class="ai-siri__subtext">{{ subtext }}</p>
      </div>
      <slot v-else />
    </div>
  </div>
 ...
 * @description 组件唯一标识
 */
const renderId = ref(props.renderId || (props.id ? props.id : "pa-ai-siri_" + useRenderId()));
...
uniform vec2 iResolution; uniform float iTime; uniform vec3 uColor;
...
function getWaveColor(): [number, number, number] {
  if (props.color) {
    return hexToRgb(props.color);
  }
  const isFluidDots = props.variant === "fluid-dots";

  if (props.dark !== undefined) {
    if (isFluidDots) {
      return props.dark ? [0, 0, 0] : [1, 1, 1];
    }
    return [1, 1, 1];
  }
  ...
  const themeBg = getComputedStyle(containerRef.value).getPropertyValue("--pa-color-bg").trim() || "#282828";
  ...
  if (isFluidDots) {
    return luminance > 0.5 ? [1, 1, 1] : [0, 0, 0];
  }
  return [1, 1, 1];
}
...
function initWebGL() {
  ...
  const fs = compileShader(gl, gl.FRAGMENT_SHADER, FRAGMENT_SHADERS[props.variant]);
  ...
  const dim = Math.round(props.size * props.renderScale);
  canvas.width = dim;
  canvas.height = dim;
...
watch(
  () => [props.variant, props.dark, props.color],
  () => {
    disposeWebGL();
    initWebGL();
  }
...
.ai-siri__text {
  ...
  animation: ai-siri-pulse 1.8s ease-in-out infinite;
}
