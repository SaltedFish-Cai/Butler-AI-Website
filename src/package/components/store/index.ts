...
export const useBaseStore = defineStore({
  ...
  getters: {
    ...
    getDictionary: (state: toolsState) => {
      return (key: string): PaOptionType.Select[] | undefined => {
    ...
    getLanguagePackage: (state: GlobalState) => {
      return (key: string) => languageMap[state.language || "zh-CN"]?.[key] || {};
    }
  },
  actions: {
     ...
     * ```
     * */
    setDictionary(key: string, data: PaOptionType.Select[]) {
