<template>
  <div :id="props.id" :class="['sa-color', { 'sa-color-disabled': props.disabled }]" :style="props.style">
    <sa-popover ref="popoverRef" :disabled="props.disabled" autoWidth @change="e => (isPickerOpen = e)">
      <template #reference>
        <!-- 颜色显示区域 -->
        <div class="sa-color-preview" :class="{ 'sa-color-preview-active': isPickerOpen }" v-if="!props.disabled">
          <div class="sa-color-preview-color" :style="{ backgroundColor: currentColor }">
            <div class="sa-color-preview-mask" v-if="!isHexColor(currentColor)"></div>
          </div>
          <div class="sa-color-preview-text">{{ currentColor }}</div>
        </div>
      </template>
      <sa-color-item v-model="currentColor"></sa-color-item>
    </sa-popover>
  </div>
</template>
