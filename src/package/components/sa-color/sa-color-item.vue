<template>
  <!-- 颜色选择器面板 -->
  <div class="sa-color-picker" v-if="!props.disabled">
    <!-- 颜色选择区域 -->
    <div class="sa-color-picker-main">
      <div class="sa-color-picker-color-area" @mousedown="onColorAreaMouseDown">
        <div class="sa-color-picker-color-area-overlay" :style="{ backgroundColor: hueColor }"></div>
        <div class="sa-color-picker-color-area-mask"></div>
        <button class="sa-color-picker-color-area-pointer" :style="{ left: `${saturation}%`, top: `${100 - value}%` }"></button>
      </div>

      <!-- 色相选择条 -->
      <div class="sa-color-picker-hue-area">
        <div class="sa-color-picker-hue-area-gradient" @mousedown="onHueAreaMouseDown"></div>
        <div class="sa-color-picker-hue-area-pointer" :style="{ left: `${(hue / 360) * 100}%` }"></div>
      </div>

      <!-- 透明度选择条 -->
      <div class="sa-color-picker-alpha-area" v-if="props.showAlpha">
        <div
          class="sa-color-picker-alpha-gradient"
          :style="{ backgroundColor: currentColorWithoutAlpha }"
          @mousedown="onAlphaAreaMouseDown"
        ></div>
        <div class="sa-color-picker-alpha-pointer" :style="{ top: `${100 - alpha * 100}%` }"></div>
      </div>
    </div>

    <!-- 颜色值输入区域 -->
    <div class="sa-color-picker-inputs">
      <div class="sa-color-picker-inputs-group" v-if="props.showInput">
        <input
          type="text"
          class="sa-color-picker-inputs-input"
          v-model="hexInput"
          @input="onHexInputChange"
          placeholder="#000000"
        />
      </div>
      <div class="sa-color-picker-inputs-group" v-if="props.showAlpha">
        <input
          type="number"
          class="sa-color-picker-input"
    ...
    <!-- 预设颜色列表 -->
    <div class="sa-color-picker-presets" v-if="presetColors && presetColors.length > 0">
      <button
        class="sa-color-picker-presets-preset"
...
import { SaColorType } from "./type";

// Props定义
const props = withDefaults(defineProps<SaColorType>(), {
  id: undefined,
  class: undefined,
  style: undefined,
  modelValue: "#000000",
