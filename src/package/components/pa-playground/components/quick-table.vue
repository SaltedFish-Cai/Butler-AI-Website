              ...
              >
                <div :style="{ width: item.prop === 'operation' ? 'max-content' : 'auto' }">
                  ...
                  (item?.filterType === "select"
                    ? (setLabel((exOptions[item.prop] as PaOptionType.SelectList)?.find(i => i.value === Operation[(item.prop as string)]) as PaOptionType.Select) || {}) || ""
                    : Operation[item.prop] || "")
                    }}
                  </slot>
                </div>
...
const setLabel = (item: PaOptionType.Select) => {
  if (!item) return "";
