...
describe("pa-icon 组件测试", () => {
  ...
  describe("1. 默认渲染", () => {
    it("渲染 i.pa-icon 和默认 span.icon-magic_line", async () => {
      ...
      expect(wrapper.find("span.pa-icon__font").exists()).toBe(true);
    });

    it("无 tip 时不渲染 popover", async () => {
      const wrapper = await mountIcon();
      expect(wrapper.find(".pa-popover-mock").exists()).toBe(false);
    });
  });
  ...
  describe("5. fontColor prop", () => {
    it("fontColor 为空数组时不添加 gradient class", async () => {
      const wrapper = await mountIcon({ fontColor: [] });
      const span = wrapper.find("span.pa-icon__font");
      expect(span.classes()).not.toContain("pa-icon__font--gradient");
    });

    it("fontColor 有值时添加 gradient class", async () => {
      const wrapper = await mountIcon({ fontColor: ["#ff0000", "#00ff00"] });
      const span = wrapper.find("span.pa-icon__font");
      expect(span.classes()).toContain("pa-icon__font--gradient");
    });

    it("fontColor 有值时设置 CSS 变量 --pa-icon-gradient", async () => {
      const wrapper = await mountIcon({ fontColor: ["#ff0000", "#00ff00"] });
      const span = wrapper.find("span.pa-icon__font");
      const style = span.attributes("style");
      expect(style).toContain("--pa-icon-gradient");
    ...
    it("fontColor 无值时 span 无 inline style", async () => {
      ...
      const span = wrapper.find("span.pa-icon__font");
      const style = span.attributes("style");
      // iconFontStyle 返回 undefined 时 Vue 不渲染 style 属性
  ...
  // ==================== tip prop ====================
  describe("6. tip prop", () => {
    it("tip 为字符串时渲染 popover 并显示文字", async () => {
      const wrapper = await mountIcon({ tip: "提示文字" });
      // 异步 popover 通过 stub 渲染
      const popover = wrapper.find(".pa-popover-mock");
