<template>
  ...
  >
    ...
    >
        ...
        <div class="sa-cascader-content">
          ...
          <div class="sa-cascader-input" :class="[isFocus ? 'is-focus' : '']">
            ...
            <input
              ...
              @input="handleInput"
            />
            <sa-icon v-if="inValue && clearable" name="close_circle_line" class="clear-icon" @click="clearInput" />
            <sa-icon :class="!isFocus ? 'down-icon' : 'down-icon up-icon'" name="down_line"></sa-icon>
          </div>
        </div>
      </template>
      <div class="sa-cascader-options" ref="optionsRef" v-if="!props.disabled && filterOptionsList.length > 0">
        <!-- <sa-scrollbar noBackTop :useShadow="false" :style="{ height: OptionsHeight }"> -->
        <div class="sa-cascader-options-group">
          <sa-cascader-option
            v-if="!filterValue"
      ...
      <div v-else-if="exOptionsList.length" class="sa-cascader-no-data">{{ languagePackage["empytFind"] }}</div>
