...
describe("pa-scrollbar-list 组件测试", () => {
  ...
  describe("15. footer slot 测试", () => {
    it("渲染 footer slot", async () => {
      ...
      const wrapper = await mountScrollbarList({}, { footer: '<div class="custom-footer">底部</div>' });
  ...
  describe("16. footerLeft slot 测试", () => {
    it("渲染 footerLeft slot", async () => {
      ...
      const wrapper = await mountScrollbarList({}, { footerLeft: '<div class="footer-left">左侧</div>' });
