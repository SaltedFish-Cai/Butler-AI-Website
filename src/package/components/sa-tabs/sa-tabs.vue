<template>
  <div
    ...
    ref="tabsRef"
  >
    <div :class="['sa-tabs-content', mode === 'portrait' || mode === 'slider' ? 'flex' : 'flex-col']">
      <div
      ...
      <div v-if="mode === 'slider'" class="sa-tabs-scroll">
        <slot name="afterLabel"></slot>

        <div :id="tabsId + '-tab-contents-slider'" class="sa-tabs-contents">
          <sa-scrollbar ref="mScrollRef" :intersectClassName="'.tab-item_line_' + tabsId" @intersecting="handleIntersecting">
            <slot></slot>
          </sa-scrollbar>
        </div>
      </div>
      <div v-else class="sa-tabs-scroll">
        <slot name="afterLabel"></slot>

        <div
          :id="tabsId + '-tab-contents'"
          class="sa-tabs-contents"
...
function setTabItemPosition() {
  nextTick(() => {
    const targetEl = document.querySelector(`#${tabsId.value} .sa-tabs-title_action_${tabsId.value}`);
    const parentElement = document.querySelector(`#${tabsId.value} #sa-tabs-box_${tabsId.value}`);
  ...
  if (tabsRef.value) {
    ...
    function setChild(arrayData) {
      for (let index = 0; index < arrayData.length; index++) {
        ...
        } else if (element.type?.__file?.includes("sa-tabs-item-")) {
