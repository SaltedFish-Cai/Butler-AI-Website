...
export const useStateHooks = (
  ...
  // # Function  解析·校验规则
  function setRule(item: SaFormChildType) {
    const baseRules =
   ...
   * @param {Array<SaTableItemType>} _config - 表格配置
   */
  function setTableConfig(_config: Array<SaTableItemType & SaTableUseItemType>, callback?: () => void) {
    const config = cloneDeep(_config);
    const stringRowIndexWidth = setWidthToString(rowIndexWidth);
    const list: Array<SaTableItemType & SaTableUseItemType> = [
      ...config.map(item => {
        if (item.cellConfig) {
        ...
        const data: SaTableItemType & SaTableUseItemType = {
          ...item,
    ...
    // @ 添加左侧固定列
    const leftFixedItem: SaTableItemType & SaTableUseItemType = {
  ...
  setTableConfig(props.structure as Array<SaTableItemType & SaTableUseItemType>);
  ...
  function getSummary() {
    ...
    const sums: string[] = [];
    const exText = "";
    tableStructure.value.forEach((column: SaTableItemType, index) => {
      if (index === 0) {
