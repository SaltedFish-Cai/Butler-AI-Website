  ...
  async function getSubmitTableList() {
    ...
    if (formResult) {
      ...
      nextTick(() => {
        ...
        const errorItem = document.querySelector(`#${props.id} .sa-table-cell__error`);

        if (errorItem) {
          const fixedIndex = document.querySelectorAll(`#${props.id} > .sa-table_body_header_box .sticky-left`);
