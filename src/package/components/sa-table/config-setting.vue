  ...
  <sa-drawer :title="languagePackage?.['colSetting'] || ''" v-model="drawerVisible" @close="handleCloseDrawer" width="680px">
      ...
      >
        <template #searchCriteria="scope">
          ...
          <template v-else-if="isNumberType(scope.row, display) || isTextType(scope.row, display)">
            ...
            ></sa-number>
            <template v-else>
              <sa-input v-model="scope.row.searchCriteria" :placeholder="languagePackage?.['inputPlaceholder']" />
              <!-- !scope.row.useSenior -->
              <sa-button v-if="false" style="width: 100%" is="edit" @click="openSeniorFilter(scope.row)">
        ...
        <template #fixed="scope">
          ...
          >
            <sa-icon name="pin_line" :class="['mr5']" />
        ...
        <template #isShow="scope">
          ...
          >
            <sa-icon class="mr5" :name="scope.row.isShow ? 'eye_line' : 'eye_close_line'"></sa-icon>
    ...
    <template #footer>
      <div class="flex-center">
        <sa-button plain type="primary" font="save_line" @click="FetchSaveAndFilter">
...
// # Var
const injectSetTableConfig = inject("setTableConfig") as (config: Array<SaTableUseItemType>) => void;
...
async function FetchSaveAndFilter() {
  ...
  for (let index = 0; index < array.length; index++) {
    const element: SaTableUseItemType = array[index];
  ...
  const useOperation = props.tableStructure?.find?.(item => item.prop == "operation") as SaTableUseItemType;
  if (useOperation) {
