...
export type SaTableExDependentType = {
...
export type SaTableCellExDependentType = {
   ...
   * */
  tag_click?: { [x: string]: (params: { row: SaTableUseType.SaTableInDataType }) => void };
...
export interface SaTableType {
   ...
   * ```
   * */
  structure: Array<SaTableItemType>;
   ...
   * ```
   * */
  exDependent?: SaTableExDependentType;
   ...
   * ```
   * */
  exCellDependent?: SaTableCellExDependentType;
...
export type SaTableItemType = {
  ...
  /**
   * **单元格配置**
   * @type `SaFormChildType`
   * @description 当设置该值为 `SaFormChildType` 时，会使用该值作为单元格配置
   * */
  cellConfig?: SaFormChildType;
...
export type SaTableUseItemType = SaTableItemType & {
  label?: string;
  /**
   * **最小宽度（当元素为flex宽元素时使用）**
   * @type `string`
...
export namespace SaTableUseType {
  ...
  export type dataType = Array<SaTableInDataType>;

  export type SaTableInDataType = {
  ...
  export type TableStateType = {
     ...
     * */
    selectTableData: Array<SaTableInDataType>;
    /**
     * **表格数据（汇总数据）**
     * @type `(string | number)[]`
     * @description 表格数据
