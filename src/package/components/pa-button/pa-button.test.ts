...
describe("pa-button 组件测试", () => {
  ...
  describe("12. 确认弹窗事件", () => {
    it("deleteClick 监听时弹出 danger 确认框", async () => {
      ...
      await wrapper.find("button").trigger("click");
      expect(M_MessageBox.confirm).toHaveBeenCalledWith(expect.objectContaining({ type: "danger" }));
    });
    ...
    it("submitClick 监听时弹出 warning 确认框", async () => {
      ...
      await wrapper.find("button").trigger("click");
      expect(M_MessageBox.confirm).toHaveBeenCalledWith(expect.objectContaining({ type: "warning" }));
    });
    ...
    it("confirmClick 监听时弹出 success 确认框", async () => {
      ...
      await wrapper.find("button").trigger("click");
      expect(M_MessageBox.confirm).toHaveBeenCalledWith(expect.objectContaining({ type: "success" }));
    });
