...
export type ComponentProps = {
  ...
  /**
   * **外置数据**
   * @type `PaOptionType.SelectList`
   * @description 当设置该值时，会使用该值作为配置数据
   * @description 该值的类型为 `PaOptionType.SelectList`，可以是任意类型
   * @example
   * ```tsx
   * <MoSelect exOptions={[]} />
   * ```
  ...
  /**
   * **远端请求选项接口**
   * @type `({ query: string }) => Promise<PaOptionType.SelectList>`
   * @description 当设置该值时，会使用该接口请求选项
   * @description 该值的类型为 `({ query: string }) => Promise<PaOptionType.SelectList>`，可以是任意类型
   * @description 该接口的参数为 `{ query: string }`，返回值为 `Promise<PaOptionType.SelectList>`
   * @example
   * ```tsx
