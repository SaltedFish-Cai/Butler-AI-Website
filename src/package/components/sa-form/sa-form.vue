<template>
  <sa-development :id="id">
    ...
    <template v-if="initialization == 1">
      <div class="sa-form" :class="[props.class]" :style="{ ...props.style }">
        <sa-form-control
          :id="id || 'default'"
          ref="FormControlRef"
        ...
        >
          <template v-for="(itemConfigs, itemConfigsIndex) in inMultipleConfig" :key="itemConfigs.unitName">
            ...
            <template v-if="itemConfigs.unitName != 'default'">
              <sa-title :padding="[itemConfigsIndex != 0 ? 'top' : 'null']">
                <div ref="RefUnitContainer">{{ itemConfigs.unitName }}</div>
                <sa-popover v-if="itemConfigs.unitTip" trigger="hover" :teleport-to="RefUnitContainer" placement="top">
                  <template #reference>
...
// const structure: SaFormItemType[] = [];
const baseFormData: Record<string, Array<string> | string> = {};
const inConfig: Ref<SaFormItemType[]> = ref(cloneDeep(props.structure || []));
...
// # Function  解析·校验规则
function setRule(
  item: SaFormChildType | SaFormItemType,
  type = "default",
...
function setMultipleConfig(configItem: ExMultipleConfigType, baseIndex: number) {
  ...
  if (configItem.tabsFormConfig?.length) {
    ...
    configItem.tabsFormConfig.map((value: SaFormChildType, configIndex: number) => {
      const item: SaFormItemType = { ...value };
      ...
      if (_tabsIndex >= 0 && configItem?.inMultipleConfig) {
        let findItem = configItem?.inMultipleConfig[_tabsIndex].configs.find((value: SaFormItemType) => value.prop == item.prop);
        if (findItem) {
          findItem = item;
        } else {
  ...
  if (_index >= 0) {
    const findIndex = inMultipleConfig[_index].configs.findIndex((value: SaFormItemType) => value.prop == configItem.prop);
    if (findIndex > -1) {
...
async function getSubmitForm() {
  ...
  if (formResult) {
    ...
    const errorItem = document.querySelector(`#${props.id} .sa-form-item.is-error`);
...
// # Expose 重置结构配置
function setStructure_All(newConfig: Array<SaFormItemType>) {
...
/**
 * **重置单个结构配置**
 * @type `(prop: string, item: SaFormItemType) => void`
 ...
 * ```
 * */
function setStructure_Item(prop: string, item: SaFormItemType) {
  if (!prop) return;
