...
export type SaFormProps = {
   ...
   * @example
   * ```ts
   * <sa-form v-model:value={{ name: "张三", age: 18 }} />
   * ```
   * */
  data?: FormDataType;
   ...
   * @example
   * ```ts
   * <sa-form contrastData={{ name: "张三", age: 18 }} />
   * ```
   * */
  contrastData?: {};
  ...
  /**
   * **表单结构**
   * @type `Array<SaFormItemType>`
   * @description 当设置该值为 `Array<SaFormItemType>` 时，会使用该值作为表单结构
   * @example
   * ```ts
   * <sa-form structure={[{ label: "姓名", prop: "name" }]} />
   * ```
   * */
  structure: Array<SaFormItemType>;
  ...
  /**
   * **表单额外依赖**
   * @type `SaFormExDependentType`
   * @description 当设置该值时，会使用该值作为表单额外依赖
   * @example
   * ```ts
   * <sa-form exDependent={{ disabledRule: { name: (value) => value === "张三" } }} />
   * ```
   * */
   ...
   * @example
   * ```ts
   * <sa-form @formCellChange={({prop, value, oldValue, option}) => { console.log(prop, value, oldValue, option) }} />
   * ```
   * */
   ...
   * @example
   * ```ts
   * <sa-form @formDataChange={(data) => { console.log("formDataChange", data) }} />
   * ```
   * */
...
export type SaFormRef = {
   ...
   * ```
   * */
  setStructure_All: (newConfig: SaFormItemType[]) => void;
  /**
   * **替换表单结构**
   ...
   * ```
   * */
  setStructureItem: (prop: string, item: SaFormItemType) => void;
  /**
   * **重置表单数据**
...
//   tabsFormConfig: (SaFormChildType & { spanStyle?: spanStyleType })[];
// };
...
export type ExMultipleConfigType = SaFormItemType & {
...
export type SaFormExDependentType = {
