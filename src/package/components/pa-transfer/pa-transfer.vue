    ...
    <div class="transfer-select-box">
      ...
      <div class="transfer-select-box_options">
        <pa-scrollbar
          ...
          :useScrollX="false"
          :padding="['top', 'bottom']"
          paddingWidth="calc(var(--pa-size-padding, 10px) / 2)"
        >
          <pa-empty
...
function handleTransferClick(direction: string): void {
  ...
  if (direction === "left") {
    ...
    selectedList.value = [
      ...selectedList.value,
      ...awaitSelectList.value.map(item => ({ value: item, label: findDataWithSelect(item, exOptionsList.value) }))
    ];
    searchAll.value = "";
...
function findData(data: Array<boolean | number | string>): string {
  ...
  return findDataWithSelect(data, exOptionsList.value);
}
/**
 * 组件挂载
