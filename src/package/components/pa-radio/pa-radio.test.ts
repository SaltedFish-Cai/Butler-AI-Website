...
// Mock pa-radio-item component
const PaRadioItemMock = defineComponent({
  name: "PaRadioItem",
  props: {
  ...
  setup(props, { emit }) {
    return () =>
      h(
        "div",
        {
          class: ["pa-radio-item", { "is-checked": props.isChecked, "is-disabled": props.disabled }],
          onClick: () => {
            if (!props.disabled) {
...
/** 通用 mount 辅助函数 */
async function mountRadio(props: Record<string, any> = {}, provideOverride: Record<string, any> = {}) {
  const { default: PaRadio } = await import("./pa-radio.vue");
  return mount(PaRadio, {
    props,
    global: {
      stubs: { "pa-radio-item": PaRadioItemMock, "pa-popover": true },
      provide: {
...
describe("pa-radio 组件测试", () => {
  ...
  // ==================== 渲染 ====================
  describe("1. 默认渲染", () => {
    it("渲染 div.pa-radio", async () => {
      const wrapper = await mountRadio();
      expect(wrapper.find("div.pa-radio").exists()).toBe(true);
    });
    ...
    it("渲染选项列表", async () => {
      ...
      expect(wrapper.findAllComponents({ name: "PaRadioItem" }).length).toBe(2);
    });
  });
  ...
  describe("2. v-model 双向绑定", () => {
    it("初始值绑定", async () => {
      const wrapper = await mountRadio({ modelValue: 1 });
    ...
    it("更新值触发 update:modelValue", async () => {
      ...
      const radioItems = wrapper.findAllComponents({ name: "PaRadioItem" });
  ...
  describe("3. disabled prop", () => {
    it("disabled=true 时选项禁用", async () => {
      const wrapper = await mountRadio({ disabled: true });
      expect(wrapper.find(".pa-radio").attributes("disabled")).toBeDefined();
    });
  });
  ...
  describe("4. exOptions prop", () => {
    it("渲染选项", async () => {
      ...
      const wrapper = await mountRadio({ exOptions: options });
      expect(wrapper.findAllComponents({ name: "PaRadioItem" }).length).toBe(2);
    });
    ...
    it("字符串 label", async () => {
      ...
      expect(wrapper.findComponent({ name: "PaRadioItem" }).text()).toContain("中文");
    });
  });
  ...
  describe("7. contrastData prop 对比数据", () => {
    it("值不同时显示对比数据", async () => {
      const wrapper = await mountRadio({ modelValue: 1, contrastData: 2 });
  ...
  describe("8. class/style prop", () => {
    it("自定义 class 应用", async () => {
      const wrapper = await mountRadio({ class: "custom-class" });
      expect(wrapper.find(".pa-radio").classes()).toContain("custom-class");
    });

    it("自定义 style 应用", async () => {
      const wrapper = await mountRadio({ style: { color: "red" } });
      const style = wrapper.find(".pa-radio").attributes("style");
  ...
  describe("9. title prop", () => {
    it("title 显示标签", async () => {
      const wrapper = await mountRadio({ title: "水果" });
      expect(wrapper.find(".pa-cell-label").text()).toContain("水果");
    });
  });
