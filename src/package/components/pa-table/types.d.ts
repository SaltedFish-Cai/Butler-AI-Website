/**
 * @module pa-table/types
 * @description PaTable 类型定义
 */
/**
...
export type PaTableCellExDependentType = {
   ...
   */
  select_RequestApi?: { [x: string]: ({ query: string }) => Promise<PaOptionType.SelectList> };
  /**
   * 时间选择器禁用日期函数
   ...
   */
  tag_click?: { [x: string]: (params: { row: PaTableUseType.PaTableInDataType }) => void };
...
export interface ComponentProps {
   ...
   * ```
   */
  exDependent?: PaTableExDependentType;
   ...
   * ```
   */
  exCellDependent?: PaTableCellExDependentType;
...
export type ComponentItemProps = {
  ...
  /**
   * 单元格配置
   * @type PaFormChildType
   * @description 当设置该值为 `PaFormChildType` 时，会使用该值作为单元格配置
   */
  cellConfig?: PaFormChildType;
...
export namespace PaTableUseType {
  ...
  export type TableStateType = {
     ...
     */
    selectTableData: Array<PaTableInDataType>;
    /**
     * 表格数据（汇总数据）
     * @type (string | number)[]
     * @description 表格数据
