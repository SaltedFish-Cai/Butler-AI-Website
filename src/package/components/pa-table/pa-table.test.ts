...
const mockComponents = [
  'pa-development', 'pa-scrollbar', 'pa-checkbox-item', 'pa-icon', 'm-icon',
  'pa-overlay', 'pa-button', 'pa-dialog', 'pa-empty', 'pa-table-cell',
  'pa-table-filter', 'senior-filter', 'operation', 'cell-tag', 'loading-1',
  'pagination', 'pa-pagination', 'config-setting', 'header-item',
  'header-item-filter', 'mTableV2Filter', 'mLightTableCell'
]
...
vi.mock('./hooks/use-observer-hooks', () => ({
  useObserverHooks: vi.fn().mockReturnValue({
    isIntersectingList: ref([]), observer: null,
    observe: vi.fn(), unobserve: vi.fn(), disconnect: vi.fn(),
    listenCellInView: vi.fn(), listenCellChildChange: vi.fn(), clearListen: vi.fn()
  })
}))
...
vi.mock('./hooks/use-state-hooks', () => ({
  useStateHooks: vi.fn().mockReturnValue({
    state: {
      tableLoadingSize: 100, tableData: [], flatTableData: [], selectTableData: [],
      summaryData: [], PageNum: 1, maxPage: 0,
      pageable: { PageNum: 1, PageSize: 10, pageSizes: [10, 20, 50], total: 0 },
      tableQuery: {}, setCellWidthIng: false, tableLoadStatus: false,
      tableLoadEndStatus: false, useOrderPropName: '', isRowOpenStatus: false,
      showSelectList: false, hoveredRowIndex: -1, hoveredColumnIndex: -1,
      listenCellInViewIng: false, oldPageIndex: 0, useAverageWidth: 0,
      inRules: {}, awaitSelectData: []
    },
    tableStructure: ref([]), isRowIndex: vi.fn().mockReturnValue(false),
    getTableList: vi.fn().mockResolvedValue({ Data: { List: [], TotalCount: 0 } }),
    handleSearch: vi.fn(), handleReset: vi.fn(), handleRefresh: vi.fn(),
    handleSortChange: vi.fn(), handleColSetting: vi.fn(),
    handleSelectAllStatus: vi.fn(), changeRowStatus: vi.fn(), changeRowAllStatus: vi.fn()
  })
}))

vi.mock('./hooks/use-select-hooks', () => ({
  useSelectHooks: vi.fn().mockReturnValue({
    handleSelectionChange: vi.fn(), handleRadioChange: vi.fn(),
    isTableSelectAll: false, selectedRowsLength: 0
  })
}))
...
vi.mock('./hooks/use-table-hooks', () => ({
  useTableHooks: vi.fn().mockReturnValue({
    tableData: ref([]), flatTableData: ref([]), loading: ref(false)
  })
}))

vi.mock('./hooks/use-column-hooks', () => ({
  useColumnHooks: vi.fn().mockReturnValue({
    tableStructure: ref([]), addColumn: vi.fn(), removeColumn: vi.fn()
  })
}))
