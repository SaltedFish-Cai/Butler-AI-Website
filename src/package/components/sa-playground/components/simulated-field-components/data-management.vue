<template>
  <m-quick-table ref="visibleTableRef" :tableConfig="tableConfig" :tableData="tableData" :exOptions="exOption">
    <template #Header>
      <sa-button is="sync" @click="handleSync">{{ language === "en-US" ? "Sync" : "表同步" }}</sa-button>
    </template>
    <template #operation="{ data }">
      <sa-button is="view" @click="handleView(data as MStructureType)">
...
const exOptionsTable2 = ref<SaOptionType.Default>({
  valueType: [
...
const tableConfig2 = ref<SaStructureType.TableV2[]>([]);
