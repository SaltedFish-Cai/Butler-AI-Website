<template>
    ...
    <template #Header>
      <sa-button
        ...
        @click="
          ...
          editId = '';
        "
        >{{ language === "en-US" ? "New Data" : "新建数据" }}</sa-button
      >
    </template>
    <template #operation="{ data }">
      <sa-button is="edit" @click="handleEdit(data.id)"> {{ language === "en-US" ? "Edit" : "编辑" }} </sa-button>
      <sa-button is="delete" @click="handleDelete(data.id)"> {{ language === "en-US" ? "Delete" : "删除" }} </sa-button>
    </template>
  </m-quick-table>
  ...
  >
    ...
    <template #footer>
      <sa-button is="save" @click="handleSubmit">
...
const formConfig = computed<SaStructureType.FormV2[]>(() => {
  return [
    {
      prop: "apiUrlId",
...
const exTableOptions = ref<SaOptionType.Default>({
