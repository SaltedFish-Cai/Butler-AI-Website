...
export type PaTabsType = {
   ...
   * @example
   * ```tsx
   * <PaTabs onTabChange={(name, index) => { console.log(name, index) }} />
   * ```
   * */
  onTabChange?: ({ name: string, index: number }) => void;
};

export type PaTabsItemType = {
  /**
   * **Tab名称**
   * @type `number` | `string`
   ...
   * @example
   * ```tsx
   * <PaTabsItem lazy={true} />
   * <PaTabsItem lazy={1000} />
   * ```
   * */
  lazy?: boolean | number;
   ...
   * @example
   * ```tsx
   * <PaTabsItem useBorder={true} />
   * ```
   * */
  useBorder?: boolean;
};

export type PaTabsItemTitleType = {
  slots: any;
  activeName: string;
  changeTabs: any;
  portrait: boolean;
